...
namespace Skywalker.Utils.Logger.IPS {
    //this is a logger that will create a list of Tuple<int, int>
    ...
    //let it run:
    //MarvelMind mm = new MarvelMind(byte_reader, IntIntListLogger)
    //while(mm.Bytes_to_Read){
    // continue;
    //}
    //List<Tuple<int, int>> your_list = logger.Values
    //run your tests on the list
    class IntIntListLogger : ILogger {

                private List<Tuple<int, int>> _list;

        //nothing needed for constructor
        public IntIntListLogger(){
        ...
        public List<Tuple<int, int>> Values {
